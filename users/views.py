...
from .serializers import FirstLoadSerializer, UserContactSerializer, CustomUserSerializer, EmailVerificationSerializer, CustomTokenObtainPairSerializer, UserMiniSerializer, UserDetailsSerializer, ResetPasswordEmailRequestSerializer, SetNewPasswordSerializer, UserUpdateSerializer,RegistrationConfigSerializer
...
class RegistrationSettings(generics.RetrieveAPIView):
    def get_object(self):
       return RegistrationConfig.objects.get(is_active=True)
    serializer_class =RegistrationConfigSerializer
    ...
    def perform_create(self, serializer):
        if serializer.is_valid():
            ...
            if settings.is_email_confirmation:
                ...
                absurl = 'http://'+'https://papouf-backend-api.herokuapp.com/api-user/email-verify/'+"?token="+str(tokens['access'])
                ...
                send_mail('Papouf email verification', Subject, 'storepapouf@gmail.com',[user_data['email']], fail_silently=False)
            else :
